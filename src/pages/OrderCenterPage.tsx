...
import { materialApi, type OrderImportBatchDto, type OrderItemDto } from '@/services/apiClient';
                          ...
                          <tr key={item.id} className="border-b border-gray-50 align-top hover:bg-gray-50/50">
                            ...
                            <td className="px-2 py-1.5">
                              <span className={`rounded px-1.5 py-0.5 text-[10px] ${
                                p.imageType === 'UNKNOWN' ? 'bg-red-50 text-red-600' : 'bg-gray-100 text-gray-600'
                              }`}>
                                {TYPE_LABEL[String(p.imageType)] ?? String(p.imageType ?? '—')}
                              </span>
                            </td>
    ...
    void (async () => {
      try {
        const [list, variants] = await Promise.all([
          materialApi.matchReview({ scope }),
          materialApi.listDesignPackages({ withBatch: true }).then((pkgs) =>
            Promise.all(
              pkgs.slice(0, 3).map((p) =>
                materialApi.getPackageOverview(p.id).then((ov) =>
                  (ov.variants ?? []).map((v) => ({ id: v.id, label: `${p.name} / ${v.displayCode}` })),
                ),
              ),
            ),
          ),
        ...
        if (cancelled) return;
        setItems(list);
        setVariantOptions(variants.flat());
      } catch (error) {
        ...
        items.map((item) => {
          ...
          return (
            <div key={item.id} className="rounded-md border border-gray-100 bg-white p-3">
              ...
              {item.matchMethod && (
                <div className="mt-0.5 text-[10px] text-gray-400">判定依据：{item.matchMethod}（{item.matchScore ?? '—'}）</div>
              )}
