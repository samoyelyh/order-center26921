    ...
    void (async () => {
      try {
        const [list, variants] = await Promise.all([
          ...
          // Variant 候选经 order-center 素材平台契约代理获取（不直接查素材库）
          materialApi.materialPlatformVariants().then((vs) =>
            vs.map((v) => ({ id: v.variantId, label: `${v.designPackageName ?? '设计包'} / ${v.displayCode ?? v.variantId}` })),
          ),
        ]);
        if (cancelled) return;
        setItems(list);
