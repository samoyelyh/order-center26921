...
//
// 只负责订单域 HTTP：
//   - 统一 base url（跟随页面 hostname:8000）
...
async function request<T>(url: string, init?: RequestInit): Promise<T> {
  ...
  if (!res.ok) {
    ...
    throw new ApiError(detail.code ?? 'HTTP_ERROR', detail.message ?? `请求失败（${res.status}）`)
  }
...
export const materialApi = {
  ...
  materialPlatformVariants: () =>
    request<MaterialVariantOption[]>('/material-platform/variants'),
}
