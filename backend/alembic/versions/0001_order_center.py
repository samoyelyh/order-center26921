...
def _missing_table(bind, name: str) -> bool:
    # Alembic --sql uses a MockConnection that cannot be inspected. In offline
    # generation the migration starts from base, so every table must be emitted.
    return bool(op.get_context().as_sql) or not sa.inspect(bind).has_table(name)


def upgrade() -> None:
    bind = op.get_bind()

    if _missing_table(bind, "order_assets"):
        op.create_table(
            "order_assets",
    ...
    if _missing_table(bind, "order_import_batches"):
        op.create_table(
    ...
    if _missing_table(bind, "order_items"):
        op.create_table(
            "order_items",
            sa.Column("id", mysql.VARCHAR(64), primary_key=True),
    ...
    if _missing_table(bind, "order_import_batch_items"):
        op.create_table(
    ...
    if _missing_table(bind, "order_buyer_assets"):
        op.create_table(
            "order_buyer_assets",
            sa.Column("id", mysql.VARCHAR(64), primary_key=True),
            sa.Column("order_item_id", mysql.VARCHAR(64), nullable=False),
    ...
    if _missing_table(bind, "material_url_bindings"):
        op.create_table(
    ...
    if _missing_table(bind, "asin_variant_bindings"):
        op.create_table(
    ...
    if _missing_table(bind, "order_match_actions"):
        op.create_table(
            "order_match_actions",
            sa.Column("id", mysql.VARCHAR(64), primary_key=True),
            sa.Column("order_item_id", mysql.VARCHAR(64), nullable=False),
