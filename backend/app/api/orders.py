...
from app.db.models import OrderImportBatch, OrderImportBatchItem, OrderItem
...
def _item_dto(row: OrderItem) -> OrderItemDTO:
    return OrderItemDTO(
        ...
        categorySource=row.category_source, categoryConfidence=row.category_confidence,
        parserVersion=row.parser_version, rawJsonHash=row.raw_json_hash,
        matchedVariantId=row.matched_variant_id, matchedMaterialId=row.matched_material_id,
        matchMethod=row.match_method, matchScore=row.match_score, matchStatus=row.match_status,
...
def list_order_import_items(batch_id: str, db: Session = Depends(get_db)) -> list[OrderItemDTO]:
    from app.core.errors import NotFoundError
