            ...
            if node_type == "optioncustomization":
                ...
                for selection_index, selection in enumerate(nodes):
                    ...
                    if thumb_url or overlay_url or image_name:
                        # 保留父节点入口名和选择值，品类解析器据此识别 Add Your
                        # Logo / Front Blank 等预览入口；通用层不做业务分类。
                        label_parts = [
                            _scalar(value.get("label")),
                            _scalar(value.get("name")),
                            _scalar(selection.get("label")),
                            _scalar(selection.get("name")),
                            _scalar(selection.get("displayValue")),
                        ]
                        label = " / ".join(dict.fromkeys(p for p in label_parts if p)) or None
                        selection_path = f"{path}.optionSelection"
                        if isinstance(selections, list):
                            selection_path += f"[{selection_index}]"
                    ...
                    option_value = (
                        _scalar(selection.get("displayValue"))
                        or _scalar(selection.get("selectedValue"))
                        or _scalar(selection.get("value"))
                        or _scalar(selection.get("name"))
                    )
                    if option_value:
                ...
                label, text = _labelled_value(value)
                text = text or _scalar(value.get("displayValue")) or _scalar(value.get("name"))
                if text:
