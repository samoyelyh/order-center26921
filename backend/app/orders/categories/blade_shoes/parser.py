    ...
    def parse(self, record: GenericOrderRecord, *, category) -> NormalizedOrder:
        ...
        material = next(
            (candidate for candidate in record.image_candidates
             if candidate.thumbnail_url and candidate.overlay_url and candidate.thumbnail_url != candidate.overlay_url),
            None,
        )
        ...
        if record.image_candidates or record.buyer_logo_original or record.buyer_logo_svg:
