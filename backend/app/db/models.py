...
class OrderImportBatchItem(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("parse_status IN ('PARSED','PARTIAL','FAILED','DUPLICATE')", name="ck_order_import_batch_items_status"),
...
class AsinVariantBinding(Base):
    ...
    现有素材库无 Distribution（派发）后端，用此表替代「Child ASIN → Batch → Variant」
    的候选定位。variant_id 只存字符串，经 MaterialPlatformClient 契约校验。
    """
